<template>
	<view class="container">
		...
		<scroll-view scroll-y="true" class="favor-list" @scrolltolower="loadMoreItems">
			<view v-if="favorItems.length === 0" class="empty-state">
				<image src="/static/empty-box.png" class="empty-icon"></image>
				<text>暂无收藏商品</text>
				<button class="go-shopping-btn" @click="goShopping">去逛逛</button>
			</view>
			<view v-else class="favor-item" v-for="item in favorItems" :key="item._id">
		...
		methods: {
			async loadFavorItems() {
				try {
					...
					const favorResult = await db.collection('favor')
						.where({
