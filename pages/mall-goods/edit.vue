	...
	<view class="uni-container">
		<uni-forms ref="form" :model="formData" validate-trigger="submit" err-show-type="toast">
			<uni-forms-item name="category_id" label="商品类别 ID" required>
				<uni-easyinput placeholder="商品所属的分类 ID，参考 `opendb-mall-categories` 表。"
					v-model="formData.category_id"></uni-easyinput>
			</uni-forms-item>
			...
			<<uni-forms-item name="goods_banner_imgs" label="Banner 图">
				<uni-file-picker return-type="object" :multiple="true"
					v-model="formData.goods_banner_imgs"></uni-file-picker>
				</uni-forms-item>
				<uni-forms-item name="reviews" label="商品评价">
					<view v-for="(review, index) in formData.reviews" :key="index" class="review-item">
						<uni-easyinput type="textarea" placeholder="请输入商品评价" v-model="formData.reviews[index]"
							trim="both"></uni-easyinput>
						<button class="remove-review-btn" type="warn" size="mini" @click="removeReview(index)"
							v-if="formData.reviews.length > 1">删除</button>
					</view>
					<button class="add-review-btn" type="default" size="mini" @click="addReview">添加评价</button>
				</uni-forms-item>
				<uni-forms-item name="remain_count" label="库存数量">
					<uni-easyinput placeholder="商品的剩余库存数量。" type="number"
						v-model="formData.remain_count"></uni-easyinput>
				</uni-forms-item>
				<uni-forms-item name="month_sell_count" label="月销量">
					<uni-easyinput placeholder="商品在本月的销售数量。" type="number"
						v-model="formData.month_sell_count"></uni-easyinput>
				</uni-forms-item>
				<uni-forms-item name="total_sell_count" label="总销量">
					<uni-easyinput placeholder="商品的累计销售数量。" type="number"
						v-model="formData.total_sell_count"></uni-easyinput>
				</uni-forms-item>
				...
				<uni-forms-item name="seller_note" label="商家备注">
					<uni-easyinput placeholder="商家对商品的备注信息，仅商家可见。" v-model="formData.seller_note"
						trim="both"></uni-easyinput>
				</uni-forms-item>
...
<script>
	import {
		validator
	} from '../../js_sdk/validator/mall-goods.js';
