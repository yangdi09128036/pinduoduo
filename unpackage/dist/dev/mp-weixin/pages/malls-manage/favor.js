  ...
  methods: {
    async loadFavorItems() {
      try {
        ...
        const favorResult = await db.collection("favor").where({
        ...
        common_vendor.index.__f__("error", "at pages/malls-manage/favor.vue:140", "加载收藏商品失败:", error);
        common_vendor.index.showToast({
          title: "加载收藏商品失败",
          icon: "none"
        });
    ...
    async handleCheckout() {
      ...
      common_vendor.index.__f__("log", "at pages/malls-manage/favor.vue:182", "商品总价", totalAmount);
      const paymentData = {
        amount: totalAmount,
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/malls-manage/favor.vue:232", "创建订单失败:", error);
        common_vendor.index.showToast({
          title: "创建订单失败，请重试",
          icon: "none"
        });
    ...
    async removeFavorItem(itemId) {
      try {
        const db = common_vendor.nr.database();
        await db.collection("favor").where({
          userId: this.userInfo._id,
          productId: itemId
        }).remove();
        ...
        common_vendor.index.__f__("error", "at pages/malls-manage/favor.vue:265", "移除收藏失败:", error);
        common_vendor.index.showToast({
          title: "移除收藏失败，请重试",
          icon: "none"
        });
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    e: $data.favorItems.length === 0
  }, $data.favorItems.length === 0 ? {
    f: common_assets._imports_1$3,
    g: common_vendor.o((...args) => $options.goShopping && $options.goShopping(...args))
  } : {
    h: common_vendor.f($data.favorItems, (item, k0, i0) => {
      return common_vendor.e({
        ...
        n: "ae60636c-0-" + i0,
        o: common_vendor.o(($event) => $options.removeFavorItem(item._id), item._id),
        p: item._id
      });
    }),
    i: common_vendor.p({
...
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages/malls-manage/favor.js.map
